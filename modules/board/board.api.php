...
class boardAPI extends board {
	...
	*/
	...
	function arrangeContent($content) {
		...
		$output = new stdClass;
		if($content) {
			$output = $output = $content->gets('document_srl', 'category_srl', 'member_srl', 'nick_name', 'title', 'content', 'tags', 'readed_count', 'voted_count', 'blamed_count', 'comment_count', 'regdate', 'last_update', 'extra_vars', 'status');
	...
	function arrangeComment($comment_list) {
		...
		if(count($comment_list) > 0) {
			foreach($comment_list as $key => $val) {
				$item = null;
				$item = $item = $val->gets('comment_srl', 'parent_srl', 'depth', 'nick_name', 'content', 'is_secret', 'voted_count', 'blamed_count', 'regdate', 'last_update');
				if(!$val->isAccessible()) {
