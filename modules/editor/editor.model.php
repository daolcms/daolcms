...
class editorModel extends editor {
	 ...
	 *
	 * However, editor_sequence can be value from getNextSequence() in case of the modified or the auto-saved for file
	 * upload
	 *
	 **/
	...
	function getEditor($upload_target_srl = 0, $option = null) {
		...
		if($allow_fileupload) {
			...
			// Use the module owning the editor for upload requests
			if(!empty($option->module_srl)){
				$oModuleModel = getModel('module');
				$editor_module_info = $oModuleModel->getModuleInfoByModuleSrl($option->module_srl);
				if($editor_module_info) $option->mid = $editor_module_info->mid;
			}
			if(!empty($option->mid)){
	 ...
	 * Result of getModuleEditor() is as same as getEditor(). But getModuleEditor()uses additional settings of each
	 * module to generate an editor
	 *
