...
| [RVE-2026-2](https://github.com/rhymix/rhymix-security/issues/22) SVG 업로드 XSS | **적용 완료** | namespace prefix, 일반 `href`, `foreignObject` 등 위험 패턴을 업로드 필터에서 차단하고, 비관리자 SVG를 whitelist 기반 sanitizer로 정화한다. | SVG·XML 내용이 확장자를 위장해도 공개 이미지 경로와 직접 표시 대상에서 제외한다. sanitizer는 외부 엔티티 네트워크 접근을 막고 파싱·정화·저장 실패 시 업로드를 거부한다. | Rhymix [merge `74b95332`](https://github.com/rhymix/rhymix-security/commit/74b953328158653de2974a8e3ac2b82450e45d3b); XE [commit `d79569ec`](https://github.com/YJSoft/xe-core/commit/d79569ecce6d60ecf6b575b29af0a0a28bc98c16) |
| [RVE-2026-3](https://github.com/rhymix/rhymix-security/issues/23) 애드온 관리 RCE | **적용 완료** | 설정 저장 시 `xe_run_method`를 `run_selected`·`no_run_selected`로 제한하고, 기존 DB 값도 캐시 생성 직전에 같은 allowlist로 재검증한다. | PHP 캐시에는 문자열 연결 대신 `var_export()` 결과를 기록하여 작은따옴표나 PHP 구문이 설정값에서 실행 문맥으로 탈출하지 못하게 했다. | Rhymix [merge `ea4f116b`](https://github.com/rhymix/rhymix-security/commit/ea4f116b4c8865541af8154574764ef8a828626e), 핵심 `bf089997`; XE [commit `70a279b4`](https://github.com/YJSoft/xe-core/commit/70a279b45852420baae0bea04365395df7934220) |
| [RVE-2026-4](https://github.com/rhymix/rhymix-security/issues/24) 게시판 API 상담글 노출 | **적용 완료** | 오류 HTTP 상태에서는 JSON/XMLRPC API 후처리를 실행하지 않으며, 상담글·임시글·열람권한 없는 글은 실제 문서 대신 소속 모듈만 가진 빈 문서로 치환한다. | BoardAPI에서도 문서 존재와 접근권한을 다시 검사하고, 실패 시 본문은 `null`, 파일·댓글은 빈 배열로 반환하여 후속 처리에서 민감정보가 복원되지 않게 했다. | Rhymix [merge `f3a08ba8`](https://github.com/rhymix/rhymix-security/commit/f3a08ba8f3d79c91920877868b778be612fc1fc4); XE [commit `b6525daf`](https://github.com/YJSoft/xe-core/commit/b6525dafd6528068b0d6689aa3e8ff6f3e468919) |
...
| [RVE-2026-7](https://github.com/rhymix/rhymix-security/issues/27) 외부 썸네일 SSRF | **적용 완료** | `DocumentItem`·`CommentItem`의 썸네일 원본을 해당 글·댓글에 실제 첨부된 이미지로 제한하고, 본문 `img src`를 서버에서 요청하던 fallback을 제거했다. | 외부 URL뿐 아니라 본문에 삽입된 임의 로컬 URL도 서버가 대신 요청하지 않는다. 따라서 DNS rebinding·redirect·사설망 주소 판별에 의존하지 않고 SSRF 요청 경로 자체를 없앴다. | Rhymix [merge `aa0f3f02`](https://github.com/rhymix/rhymix-security/commit/aa0f3f020011aa4ffbba93449243de0159f5fbf4); XE 부분 구현 [commit `03c5ef9d`](https://github.com/YJSoft/xe-core/commit/03c5ef9df4a5dee005eab464b126f39fdc5be3fc) |
| [RVE-2026-8](https://github.com/rhymix/rhymix-security/issues/28) 세션 ID 미갱신 | **적용 완료** | 로그인 인증과 after 트리거가 성공한 뒤 회원 정보를 세션에 저장하기 직전에 `session_regenerate_id(true)`를 호출한다. 기존 설정이 없는 사이트도 기본 활성화된다. | 기존 XE SSO를 사용하면 호환성을 위해 재생성을 강제로 끄고, 관리자 일반 설정에서 보안 효과와 SSO 제약을 안내한다. 비활성화는 명시적으로 저장한 경우에만 허용한다. | Rhymix [merge `a0af01c7`](https://github.com/rhymix/rhymix-security/commit/a0af01c76a0388b8954eb1781f03b6e545accde7); XE [commit `2293e8cc`](https://github.com/YJSoft/xe-core/commit/2293e8cc044d53184449d9954cb84c2aea457825) |
| [RVE-2026-9](https://github.com/rhymix/rhymix-security/issues/29) 자동 링크 XSS | **적용 완료** | 자동 링크 변환 전에 `&`, `<`, `>`, `"`, `'`를 모두 HTML escape하여 URL의 따옴표가 `href` 밖으로 탈출하지 못하게 했다. | 일반·관리자 레이아웃 미리보기 메서드, action, UI·JavaScript 호출과 `files/cache/layout/tmp.tpl` 임시 컴파일 경로를 모두 제거했다. Daol에는 oEmbed 애드온이 없어 별도 수정 대상이 없다. | Rhymix [merge `877c85ff`](https://github.com/rhymix/rhymix-security/commit/877c85ff2987e514a9fa2262d413f22aeb5c8313); XE [기본 `edfd493f`](https://github.com/YJSoft/xe-core/commit/edfd493f56956fdd8e6fed03d29c744eb3ab9a53), [보충 `d0c3b527`](https://github.com/YJSoft/xe-core/commit/d0c3b52719821faab00a548480f1d5f444e03cef) |
...
| [RVE-2026-13](https://github.com/rhymix/rhymix-security/issues/33) 백슬래시 URL open redirect | **적용 완료** | 반환 URL을 먼저 decode한 뒤 raw·`%5C` 백슬래시를 `/`로 정규화하여 브라우저와 `parse_url()`의 해석 차이를 제거했다. | host가 있는 URL은 기본·현재 사이트 도메인과 비교하고, host 없이 scheme만 있는 `javascript:`·`data:` 등은 거부한다. scheme·port·IDNA 최종 정책은 RVE-2026-17에서 추가 강화한다. | Rhymix [merge `22d51041`](https://github.com/rhymix/rhymix-security/commit/22d5104176ac957ed0b75cc2a262480d3d5b9613); XE [commit `4d2c4b64`](https://github.com/YJSoft/xe-core/commit/4d2c4b647775bf4a4bc21fa11186f05e0de2eae0) |
| [RVE-2026-14](https://github.com/rhymix/rhymix-security/issues/34) importer SSRF·정보유출 | **적용 완료** | XML 경로 확인과 실제 전처리 양쪽에서 모든 `scheme://` URL을 캐시 생성 전에 거부하고 로컬 일반 파일만 연다. | cache key는 예측 불가능한 32자 hex CSPRNG 값으로 한 import의 다단계 처리 동안 재사용한다. 후속 요청 key도 형식을 검증하며 전처리 오류와 완료 시 임시 핸들·디렉터리를 정리한다. | Rhymix [merge `e13ab220`](https://github.com/rhymix/rhymix-security/commit/e13ab220f993db8a0a37a9203b4d3d20efd2d9ce); XE [commit `fb92cf49`](https://github.com/YJSoft/xe-core/commit/fb92cf49aa17c76906eb5b67bb1e48326d1c1ad2) |
| [RVE-2026-15](https://github.com/rhymix/rhymix-security/issues/35) 애드온 이름 path traversal | **적용 완료** | `selected_addon`, `addon`, `addon_name`을 view·controller·model·DB 캐시 생성 경계에서 ASCII 영숫자·밑줄로 제한하고 잘못된 요청은 다른 이름으로 정리하지 않고 거부한다. | type은 `pc`·`mobile`만 허용한다. 기존 DB의 잘못된 이름은 캐시에서 제외하고 애드온 파일 경로와 mid 목록은 `var_export()`로 PHP 캐시에 안전하게 기록한다. | Rhymix [merge `9f0823f9`](https://github.com/rhymix/rhymix-security/commit/9f0823f9c81a7de33675b937f40e892709315c92); XE [commit `2d04ba50`](https://github.com/YJSoft/xe-core/commit/2d04ba503f441c3372cfb6f5e7f42dac694b8fa0) |
