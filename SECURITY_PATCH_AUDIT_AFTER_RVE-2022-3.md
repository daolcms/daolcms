...
| RVE | 판정 | Daol 확인 결과 | 수정 방향 | 참고 커밋 |
| --- | --- | --- | --- | --- |
| [RVE-2023-1](https://github.com/rhymix/rhymix-security/issues/10) 에디터 reflected XSS | **누락 P0** | `modules/editor/editor.model.php`가 요청의 `component_list`를 신뢰하고, CKEditor·XpressEditor 템플릿에서 컴포넌트 키를 안전한 JS 데이터로 만들지 않는다. | 요청값을 무시하고 서버에서 컴포넌트 목록을 항상 다시 로드한다. CKEditor는 연관배열을 `json_encode()`하고, XpressEditor는 컴포넌트 이름을 HTML escape한다. | Rhymix [merge `7d148dcf`](https://github.com/rhymix/rhymix-security/commit/7d148dcfc7515d2e24e09218b5588dd2cfc72a21); XE [merge `5cd61c13`](https://github.com/YJSoft/xe-core/commit/5cd61c13c94f51054f7853700c6823dedb814d23), [XpressEditor `220431dd`](https://github.com/YJSoft/xe-core/commit/220431dd81157773eefbe71f20a31dc1576ab3a1) |
| [RVE-2023-2](https://github.com/rhymix/rhymix-security/issues/11) 에디터 미리보기 reflected XSS | **누락 P0** | `modules/editor/tpl/preview.html`이 `$editor_sequence`와 `$content`를 그대로 출력한다. | 사용하지 않는 `dispEditorPreview`를 제거하는 것이 최선이다. 호환성 때문에 유지하면 sequence를 `intval()`하고 content에 `removeHackTag()`를 적용한다. | Rhymix는 별도 보안 merge 없이 기능을 삭제한 [commit `8054564b`](https://github.com/rhymix/rhymix-security/commit/8054564b37548581534a6d95c162ac3dc7ad1dc9); XE 호환 [commit `36057c92`](https://github.com/YJSoft/xe-core/commit/36057c92dcc58eb64dc61a4a53f7037653b2de15) |
| [RVE-2023-3](https://github.com/rhymix/rhymix-security/issues/12) 글 수정 시 작성자 변조 | **누락 P1** | `modules/document/document.controller.php`와 `modules/comment/comment.controller.php`가 수정 요청의 작성자 필드를 충분히 원본 값으로 고정하지 않는다. | 글·댓글 수정 시 기존 작성자 정보를 먼저 복원한다. 비회원 글 또는 명시적인 내부 `manual_member_info` 경로만 예외로 두고, 트리거 호출 순서도 XE 호환 커밋과 맞춘다. | RVE-2023-4와 함께 Rhymix [merge `71e4118b`](https://github.com/rhymix/rhymix-security/commit/71e4118bd55517f0fd1a51805cecdc341c8175e0); XE [commit `25e2b443`](https://github.com/YJSoft/xe-core/commit/25e2b443985a56e90da8048ecbdf114a87322476) |
| [RVE-2023-4](https://github.com/rhymix/rhymix-security/issues/13) 문서 속성 임의 조작 | **누락 P0** | `modules/board/board.controller.php`가 요청의 `extra_vars`를 제거하지 않으며, 문서·댓글 controller가 날짜·작성자·정렬 관련 필드를 충분히 제한하지 않는다. | 게시판 요청에서 내부 컬럼(`extra_vars`, `list_order` 등)을 제거하고, 식별자·날짜·작성자 필드를 정수화 또는 권한별 allowlist로 제한한다. 서드파티 내부 호출과 HTTP 요청을 구분한다. | RVE-2023-3과 함께 Rhymix [merge `71e4118b`](https://github.com/rhymix/rhymix-security/commit/71e4118bd55517f0fd1a51805cecdc341c8175e0); XE [commit `25e2b443`](https://github.com/YJSoft/xe-core/commit/25e2b443985a56e90da8048ecbdf114a87322476) |
| [RVE-2023-5](https://github.com/rhymix/rhymix-security/issues/14) 타인의 쪽지 열람 | **누락 P0** | `modules/communication/communication.view.php`는 원본 쪽지의 발신자와 URL의 수신자만 비교하고, 현재 로그인 회원이 실제 수신자인지 확인하지 않는다. | 답장 원문을 표시할 때 `source_message->receiver_srl === logged_info->member_srl` 조건을 추가한다. | Rhymix [merge `6f124472`](https://github.com/rhymix/rhymix-security/commit/6f124472bb54657b358b3c66a8bb92fa4222f30d); XE [commit `205b9130`](https://github.com/YJSoft/xe-core/commit/205b9130bbd52c0008d7cc88bdad32a5b96c5528) |
| [RVE-2023-6](https://github.com/rhymix/rhymix-security/issues/15) 타인 게시물에 파일 첨부 | **누락 P0** | `modules/file/file.controller.php`가 요청의 `upload_target_srl`을 세션에 발급된 `editor_sequence`와 비교하지 않는다. 관련 삭제·대표이미지·파일목록 경로도 변수 간 관계 검증이 약하다. | `editor_sequence`, `module_srl`, `upload_target_srl`을 세션 발급값과 모두 일치시킨다. 삭제·대표이미지 지정 시 `file_srl`이 실제 대상에 속하는지도 검증한다. RVE-2026-12까지 반영된 최종 형태로 이식한다. | Rhymix [merge `f4474070`](https://github.com/rhymix/rhymix-security/commit/f4474070e1ecf2ab9b3b7a88d686d4ce5064a9e6); XE [1차 `7eafde00`](https://github.com/YJSoft/xe-core/commit/7eafde0053563f156f3931b8d35365290614b0d0), [2차 `0a7895b9`](https://github.com/YJSoft/xe-core/commit/0a7895b99d40747613633a67e2a023f5ce1d726e), [3차 `86ec5a9a`](https://github.com/YJSoft/xe-core/commit/86ec5a9adb0b9b0f61cbc6f18a3cb557633380ba) |

### 2024

| RVE | 판정 | Daol 확인 결과 | 수정 방향 | 참고 커밋 |
| --- | --- | --- | --- | --- |
| [RVE-2024-1](https://github.com/rhymix/rhymix-security/issues/16) 자동 생성 제목 XSS | **누락 P0** | `modules/document/document.controller.php`의 본문 기반 자동 제목이 저장 전에 escape되지 않고, `DocumentItem::getTitleText()`도 원문을 반환한다. 기본 게시판 스킨은 JS 문자열을 수작업으로 조립한다. | 수동·자동 제목을 일관되게 escape하고 `getTitleText()` 반환도 보호한다. JS에는 문자열 치환 대신 `json_encode()`를 사용한다. | Rhymix [merge `9eea7163`](https://github.com/rhymix/rhymix-security/commit/9eea71631a0b1626f5a74c4e55083d0d875e6657); XE [commit `899d842d`](https://github.com/YJSoft/xe-core/commit/899d842d50f5516b551f5ec2e799f5c8a25cfa31) |
| [RVE-2024-2](https://github.com/rhymix/rhymix-security/issues/17) 외부페이지 RCE 2탄 | **누락 P0** | `modules/page/page.admin.controller.php`가 외부페이지 `path`·`mpath`에 `files/cache` 경로를 허용한다. | 우선 `files/cache`를 금지하고, 최종적으로는 RVE-2026-10의 전체 금지 디렉터리 allowlist 정책으로 대체한다. 기존 설정값도 마이그레이션 시 검사한다. | Rhymix [merge `454a8e36`](https://github.com/rhymix/rhymix-security/commit/454a8e36b65fc99e31b49bcc57ebcf69ae6ced43); XE [commit `7a1988a8`](https://github.com/YJSoft/xe-core/commit/7a1988a83873b068d6d114ccdfff2fd436048898) |
| [RVE-2024-3](https://github.com/rhymix/rhymix-security/issues/18) 편집 레이아웃 소스 노출 | **부분 적용 P1** | `.htaccess`는 `files/faceOff/*.html`을 이미 차단한다. `nginx_rewrite.conf`는 layouts만 우회시키고 `files/faceOff` 직접 접근 차단이 없다. | Apache 규칙은 유지한다. Nginx 규칙의 템플릿 차단 대상에 `files/(faceOff|ruleset)`을 포함하고, 서브디렉터리 설치 예제도 같은 정책으로 맞춘다. | Rhymix [merge `670c7b29`](https://github.com/rhymix/rhymix-security/commit/670c7b29fafc1adfc6a5c00750f5b8f9efc6d724); 원래 XE [commit `4e57f849`](https://github.com/xpressengine/xe-core/commit/4e57f84949cc45ca5da9f091961791242954d4f2) |

### 2025

| RVE | 판정 | Daol 확인 결과 | 수정 방향 | 참고 커밋 |
| --- | --- | --- | --- | --- |
| [RVE-2025-1](https://github.com/rhymix/rhymix-security/issues/19) XML 쿼리 정렬 SQL injection | **누락 P0** | `classes/xml/xmlquery/argument/SortArgument.class.php`가 정렬값을 검증 없이 반환한다. | 동적 `<index>` 값은 `table.column` 또는 `column` 형태의 식별자만 허용한다. 유효하지 않으면 XML의 하드코딩된 default로 되돌리거나 쿼리를 실패시킨다. | Rhymix [merge `5f5cd81f`](https://github.com/rhymix/rhymix-security/commit/5f5cd81f1b06237fbae4e2bdff67c395f2e62a05); XE [commit `ef2d77e6`](https://github.com/YJSoft/xe-core/commit/ef2d77e64717a9ace2617e365bc90218893ffda7) |
| [RVE-2025-2](https://github.com/rhymix/rhymix-security/issues/20) 레이아웃 기능 임의 파일 삭제 | **누락 P1** | `modules/layout/layout.admin.controller.php::procLayoutAdminUserImageDelete()`가 `filename`을 그대로 삭제 경로에 넣는다. | `layout_srl` 존재를 확인하고, filename에서 `..`, `/`, `\\`를 거부한다. 가능하면 디렉터리 내 실제 파일명 allowlist와 `realpath()` 경계 검사도 추가한다. | 보안 브랜치 merge 없이 Rhymix 직접 [commit `197295ba`](https://github.com/rhymix/rhymix-security/commit/197295ba437ebb22bab19636ee18dd649cc178e3); XE [commit `086bbf31`](https://github.com/YJSoft/xe-core/commit/086bbf31316236b9fab0fa80712b4fd90532e897) |
...
| [RVE-2026-10](https://github.com/rhymix/rhymix-security/issues/30) 외부페이지 RCE 3탄 | **누락 P0** | 외부페이지 경로에 `files/attach`, `files/config`, `files/member_extra_info` 등 사용자 생성·민감 경로를 지정할 수 있다. | 경로 구분자와 null byte를 정규화한 뒤 `files/(attach|cache|config|debug|env|member_extra_info|ruleset|site_design|thumbnails)`를 거부한다. symlink의 `realpath()`에도 같은 검사를 재적용한다. | Rhymix [merge `899a1845`](https://github.com/rhymix/rhymix-security/commit/899a18452717c26ac72348a963d4d5ddee02186f); XE [commit `152d3bb3`](https://github.com/YJSoft/xe-core/commit/152d3bb3662e3b9c7d898e15a0cebc5fa210167f) |
...
| [RVE-2026-12](https://github.com/rhymix/rhymix-security/issues/32) 권한 없는 파일 일괄 삭제 | **누락 P0** | `procFileDelete()`가 각 `$srl`의 권한을 검사한 뒤 실제로는 원본 `$file_srl` 전체 목록을 `deleteFile()`에 넘긴다. | 각 file_srl마다 대상 소속과 삭제 권한을 검증하여 별도 `valid_file_srls` 배열을 만든 뒤 그 배열만 삭제한다. RVE-2023-6의 세션 대상 검증을 선행한다. | Rhymix [merge `eb925d9d`](https://github.com/rhymix/rhymix-security/commit/eb925d9d2857c1fdc35aa41c407da08960bf9f7a); XE [commit `8357f3f2`](https://github.com/YJSoft/xe-core/commit/8357f3f296186380d84e9bfad6dcc7d4fb26b0bd) |
