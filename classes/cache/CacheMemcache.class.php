...
class CacheMemcache extends CacheBase {
	 ...
	 * (but memcached server doesn't guarantee this item to be stored all the time, it could be delete from the cache
	 ...
	 * @param mixed  $buff        The variable to store. Strings and integers are stored as is, other types are stored
	 *                            serialized.
	 ...
	 *                            but in the latter case the number of seconds may not exceed 2592000 (30 days). If
	 *                            it's equal to zero, use the default valid time CacheMemcache::valid_time.
	 ...
	 *
	 * CacheMemcache::get() returns previously stored data if an item with such $key exists on the server at this
	 * moment.
	 *
	 * @param string $key             The key to fetch
	 * @param int    $modified_time   Unix time of data modified.
	 *                                If stored time is older then modified time, return false.
	 * @return false|mixed Return false on failure or older then modified time. Return the string associated with the
	 *                     $key on success.
	 */
