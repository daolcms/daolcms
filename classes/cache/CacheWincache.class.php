...
class CacheWincache extends CacheBase {
	 ...
	 *                            If a variable with same $key is already present the function will overwrite the
	 *                            previous value with the new one.
	 * @param mixed  $buff        Value of a variable to store. $value supports all data types except resources, such
	 *                            as file handlers.
	 * @param int    $valid_time  Time for the variable to live in the cache in seconds.
	 *                            After the value specified in ttl has passed the stored variable will be deleted from
	 *                            the cache. If no ttl is supplied, use the default valid time
	 *                            CacheWincache::valid_time.
