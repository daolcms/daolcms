...
class CacheApc extends CacheBase {
	 ...
	 *
	 * @param string $key         Store the variable using this name. $key are cache-unique, so storing a second value
	 *                            with the same $key will overwrite the original value.
	 * @param mixed  $buff        The variable to store
	 * @param int    $valid_time  Time To Live; store $buff in the cache for ttl seconds.
	 *                            After the ttl has passed., the stored variable will be expunged from the cache (on
	 *                            the next request). If no ttl is supplied, use the default valid time
	 *                            CacheApc::valid_time.
