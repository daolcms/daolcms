...
class DBMysql extends DB {
	...
	function __connect($connection){
		...
		if(!$result){
			exit('DAOL CMS cannot connect to DB.');
		}
		...
		if(version_compare(mysql_get_server_info($result), '4.1', '<')){
			$this->setError(-1, 'DAOL CMS cannot be installed under the version of mysql 4.1. Current mysql version is ' . mysql_get_server_info());
			return;
		}
		// select db
	...
	function __query($query, $connection){
		if(!$connection){
			exit('DAOL CMS cannot handle DB connection.');
		}
