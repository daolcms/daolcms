...
class DB {
	...
	function actFinish(){
		...
		foreach($bt as $no => $call){
			if($call['function'] == 'executeQuery' || $call['function'] == 'executeQueryArray'){
				...
				$log['called_file'] = str_replace(_DAOL_PATH_ , '', $log['called_file']);
				$call_no++;
	...
	*/
	...
	function checkQueryCacheFile($query_id, $xml_file){
		...
		$cache_file = sprintf('%s%s%s.%s.%s.cache.php', _DAOL_PATH_, $this->cache_file, $query_id, __ZBXE_VERSION__, $this->db_type);
